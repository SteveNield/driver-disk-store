...
function updateMakes(){
    httpClient.get('/makes').then(function(makes){
        state.makes = makes;
        publishUpdate();
...
function updateModels(makeId){
    httpClient.get('/makes/'+makeId+'/models').then(function(models){
        state.models = models;
        publishUpdate();
    }, logDataRequestFailure);
}
...
function updateOperatingSystems(){
    httpClient.get('/operatingsystems').then(function(operatingSystems){
